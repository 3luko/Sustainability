...
public class Game extends Application 
{
    ...
    private Button submitButton, next, prev, startMcqButton, startTfButton, closeGame;//yesPlay, closeGame;// noPlay, closeGame;
    ...
    public void scoreBoard() { //method to show the score board at the end of the game
    	...
    	testSCORE.getChildren().addAll(userScore, userScoreNum, thanks, buttonPlace); //putting the message, user score, thank you message and buttons for restarting and closing in the VBox
    ...
    public void startGameMultipleChoice(ActionEvent e) //method for starting the mcq questions
    {
        ...
        initializeGame();//calls the initializeGame method
    ...
    public void startGameTrueFalse(ActionEvent e)  //method for if you select startGameTrueFalse
    {
        ...
        initializeGame();//calls the initializeGame
        startGame();//calls the startGame();
    }
    
    private void initializeGame() //method for calling exactly which game you want to play
    {
        if (mcqGame) //if mcqGame is true it will call the initializemcqgame(); method else it will call the initializetfGame();
        {
            initializeMcqGame();
        }
        else if (tfGame) 
    ...
    private void startGame() //method for starting the game based on whether the user selected mcq or tf
    {
        ...
        if(mcqGame) {
        	question = new Text(mcqQuestions[currentQuestionIndex]);
        } else if(tfGame) {
        	question = new Text(tfQuestions[currentQuestionIndex]);
        }
    ...
    private void initializeRadioButtons() //method for initializing the radio buttons
    {
        group = new ToggleGroup(); //initializing a Toggle group 

        if (mcqGame) //it will initialize based on if mcq was selected or tf was selected
        {
            mcqA1 = new RadioButton("A: " + mcqOptions[currentQuestionIndex][0]);//based on index it will show option
            ...
            mcqA1.setToggleGroup(group); //adding buttons to toggle group
    ...
    private void compareAnswers() 
    {
        ...
        if (mcqGame) //check if the game is either multiple choice or tf
        {
            ...
            // Check if tfs[currentQuestionIndex] is not null
            if (tfs[currentQuestionIndex] != null) {
    ...
    */
    ...
    private void initializeTfGame() //method for initializing the tf game
    {
    	...
    	for (int i = 0; i < qTopics.length; i++) { //for loop for storing data into array
    ...
    public void closeButton(ActionEvent event) {//method for closing the program
